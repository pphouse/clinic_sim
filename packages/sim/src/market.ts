 ...
 *
 * ★医師が0名なら魅力も0。**看板だけの院に患者は来ない。**
 * ここを 0 にしないと、医師を置いていない空の院が競合を押し出せてしまう
 * （実際それで起きた。診察できないのに引力だけあるのは筋が通らない）。
 */
...
export function tickMarket(input: MarketInput): MarketOutcome {
  ...
  // ★セグメントは（商圏 × 科）。**内科の隣に皮膚科を出しても食い合わない**
  // （docs/spec/05-specialty.md §1）。ここが「科を選ぶ楽しさ」の要。
  for (const district of DISTRICTS) {
    for (const specialty of SPECIALTIES) {
      const clinics = input.clinics.filter(
        (c) =>
          c.open &&
          c.config.districtId === district.id &&
          c.config.specialtyId === specialty.id,
      );
      const rivals = active.filter(
        (c) => c.districtId === district.id && c.specialtyId === specialty.id,
      );
